...
function buildSessionBook(s) {
  ...
  const faces = [];
  faces.push(`<h1>🏋️ FormCoach AI</h1><h2>${esc(s.exercise)}</h2><p>${esc(s.shortDate)}</p>
    <p class="big">${s.reps} reps · ${s.avgScore}/100</p>
    <p>${esc(s.athlete)}'s training book.<br>Every photo analyzed on this device.<br>Tap the page or press → to turn.</p>`);
  for (const f of shots) {
    const stamp = tstamp(f.at);
    faces.push(`<img src="${f.img}" alt="training frame at ${stamp}" /><p class="cap">⏱ ${stamp}</p>`);
    faces.push(`<h3>⏱ ${stamp}</h3><p class="fault">${esc(f.text)}</p>
      ${notes[stamp] ? `<div class="vn">${fmtNote(notes[stamp])}</div>` : `<p class="vn">Timed form-check frame.</p>`}`);
  }
  faces.push(`<h3>📓 Error notebook</h3>
    ${state.errorLog.map((n) => `<p class="err">⏱ ${tstamp(n.at)} · rep ${n.rep} — ${esc(n.text)}</p>`).join("") || "<p>No faults — clean session!</p>"}`);
  faces.push(`<h3>🎙 Coach's verdict</h3><p>${esc(summarize(s))}</p>
    <p class="vn">Generated by FormCoach AI — United Hacks V7. No cloud, no uploads: written on the athlete's own device.</p>`);
  if (faces.length % 2) faces.push(`<p class="big" style="margin:auto">The End 🏁</p>`);

  const sheets = [];
  for (let k = 0; k * 2 < faces.length; k++) {
    const cover = k === 0 ? " cov" : "";
    sheets.push(`<div class="sheet" data-k="${k}">
      <div class="face front${cover}">${faces[2 * k]}</div>
      <div class="face back">${faces[2 * k + 1] || ""}</div>
    </div>`);
  }
  ...
  #book{position:relative;width:min(96vw,980px);height:min(84vh,640px);perspective:2600px}
  #book::before{content:"";position:absolute;inset:0;left:0;width:50%;background:#efe9e7;border-radius:12px 0 0 12px;box-shadow:0 10px 34px rgba(0,0,0,.4)}
  ...
  .face{position:absolute;inset:0;backface-visibility:hidden;background:#f5f1f0;box-sizing:border-box;
    padding:1.3rem;overflow:auto;display:flex;flex-direction:column;justify-content:center;text-align:center;
    border-radius:0 12px 12px 0;box-shadow:0 10px 34px rgba(0,0,0,.35)}
  .face::after{content:"";position:absolute;left:0;top:0;bottom:0;width:26px;pointer-events:none;
    background:linear-gradient(90deg,rgba(0,0,0,.14),transparent)}
  .face.back{transform:rotateY(180deg);border-radius:12px 0 0 12px}
  .face.back::after{left:auto;right:0;background:linear-gradient(-90deg,rgba(0,0,0,.14),transparent)}
  .face img{max-width:100%;max-height:82%;object-fit:contain;border-radius:10px;margin:auto;display:block}
  .cap{font-weight:700;color:#5e5ce6}
  .cov{background:linear-gradient(135deg,#5e5ce6,#7d7bff);color:#fff}
  .big{font-size:1.5rem;font-weight:700}
  .fault{color:#c0392b;font-weight:600}
  .vn{font-size:.9rem;color:#333;background:#fff;border-radius:10px;padding:.7rem;text-align:left}
  .vl{margin:.3rem 0}.vl b{color:#5e5ce6;margin-right:.3rem}
  ...
  zfix();
  const flip=(d)=>{if(d>0&&c<sh.length){sh[c++].classList.add("turned");}else if(d<0&&c>0){sh[--c].classList.remove("turned");}zfix();};
