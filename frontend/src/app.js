  ...
  for (const f of shots) {
    ...
    pages.push(`<section class="page">
      ...
      <div class="pg-right"><h3>⏱ ${stamp}</h3><p class="fault">${esc(f.text)}</p>
        ${vision ? `<p class="vn">🔍 What the AI sees: ${esc(vision)}</p>` : `<p class="vn">Timed form-check frame.</p>`}</div></section>`);
  }
  pages.push(`<section class="page">
    <div class="pg-left"><h3>📓 Error notebook</h3>
      ${state.errorLog.map((n) => `<p class="err">⏱ ${tstamp(n.at)} · rep ${n.rep} — ${esc(n.text)}</p>`).join("") || "<p>No faults — clean session!</p>"}</div>
