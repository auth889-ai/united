...
function buildSessionBook(s) {
  ...
  // match each photo with its section of the vision review (split on ⏱ headers)
  const visionBits = {};
  (s.visionNote || "").split("⏱").slice(1).forEach((part) => {
    const stamp = part.slice(0, 8).trim().split(" ")[0];
    visionBits[stamp] = part.slice(part.indexOf("\n") + 1).trim();
  });
  const pages = [];
  pages.push(`<section class="page cover"><h1>🏋️ FormCoach AI</h1>
    <h2>${esc(s.exercise)} — ${esc(s.shortDate)}</h2>
    <p class="big">${s.reps} reps · avg form ${s.avgScore}/100</p>
    <p>${esc(s.athlete)}'s training book — every photo analyzed on-device. Tap or use →</p></section>`);
  for (const f of shots) {
    const stamp = tstamp(f.at);
    const vision = visionBits[stamp];
    pages.push(`<section class="page"><h3>⏱ ${stamp} — ${esc(f.text)}</h3>
      <img src="${f.img}" alt="training frame" />
      ${vision ? `<p class="vn">🔍 ${esc(vision)}</p>` : ""}</section>`);
  }
  if (state.errorLog.length) {
    pages.push(`<section class="page"><h3>📓 Error notebook</h3>
      ${state.errorLog.map((n) => `<p class="err">⏱ ${tstamp(n.at)} · rep ${n.rep} — ${esc(n.text)}</p>`).join("")}</section>`);
  }
  pages.push(`<section class="page"><h3>🎙 Coach's verdict</h3><p>${esc(summarize(s))}</p>
    <p class="vn">Generated by FormCoach AI — United Hacks V7. No cloud, no uploads: this book was written on the athlete's own device.</p></section>`);
  ...
  #book{position:relative;width:min(92vw,520px);height:min(80vh,660px);perspective:1800px}
  .page{position:absolute;inset:0;background:#f5f1f0;border-radius:14px;padding:1.4rem;box-sizing:border-box;
    box-shadow:0 8px 30px rgba(0,0,0,.35);backface-visibility:hidden;transform-origin:left center;
    transition:transform .8s cubic-bezier(.4,.1,.2,1);overflow:auto}
  .page.flipped{transform:rotateY(-170deg);pointer-events:none}
  .page img{width:100%;border-radius:10px}
  .cover{background:#5e5ce6;color:#fff;display:flex;flex-direction:column;justify-content:center;text-align:center}
  .big{font-size:1.5rem;font-weight:700}
  .vn{font-size:.9rem;color:#444;background:#fff;border-radius:8px;padding:.6rem}
  .err{color:#c0392b;border-left:3px solid #c0392b;padding-left:.6rem;font-size:.9rem}
  h3{color:#5e5ce6;margin-top:0}
